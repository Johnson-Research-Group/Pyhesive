...
class Mesh(object):
  ...
  def BuildAdjacencyMatrix(self,cells,format="lil",v2v=False):
    ...
    cDim = len(elIds[0])
    v2c = sparse.coo_matrix((np.ones((ne*cDim,),dtype=np.intp),(cells.ravel(),elIds.ravel(),),))
  ...
  def RemapVertices(self):
    modSourceVertices,modDestVertices = [],[]
    gConv = dict()
    for part,(bdFaces,gConv) in zip(self.partitions,self.GenerateGlobalConversion(gConv)):
      partConv = np.array([[gConv[x][0][0] if x in gConv else x for x in c] for c in self.cells[part]])
      try:
      ...
      for v in vConv:
        self.log.debug("Updated previous mapping %d: %d -> %d" % (v,gConv[v][1][0],gConv[v][0][0]))
  ...
  @staticmethod
  def VerifyCohesiveMesh(msh):
    assert isinstance(msh,pyhesive.Mesh)
    msh.log.info("verifying")
    if len(msh.cohesiveCells):
      cohesiveSet = set(frozenset(cell) for cell in msh.cohesiveCells)
      msh.log.debug("Number of unique cohesive elements %s, total number of cohesive elements %s" % (len(cohesiveSet),len(msh.cohesiveCells)))
      if len(cohesiveSet) != len(msh.cohesiveCells):
        raise RuntimeError("There are duplicate cohesive cells!")
      sh.log.info("Mesh seems ok")
    else:
      self.log.info("Mesh doesn't have cohesive cells")
    return
